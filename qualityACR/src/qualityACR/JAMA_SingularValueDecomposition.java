...
public class JAMA_SingularValueDecomposition implements java.io.Serializable {
	...
	public JAMA_SingularValueDecomposition(final JAMA_Matrix Arg) {
		...
		for (int k = 0; k < Math.max(nct, nrt); k++) {
			if (k < nct) {
				...
				s[k] = 0;
				for (int i = k; i < m; i++) {
					s[k] = JAMA_Maths.hypot(s[k], A[i][k]);
				}
				if (s[k] != 0.0) {
					if (A[k][k] < 0.0) {
						s[k] = -s[k];
		...
		while (p > 0) {
			...
			case 1: {
				...
				e[p - 2] = 0.0;
				for (int j = p - 2; j >= k; j--) {
					double t = JAMA_Maths.hypot(s[j], f);
	...
	public JAMA_Matrix getU() {
		return new JAMA_Matrix(U, m, Math.min(m + 1, n));
	}
	...
	public JAMA_Matrix getV() {
		return new JAMA_Matrix(V, n, n);
	}
	...
	public JAMA_Matrix getS() {
		final JAMA_Matrix X = new JAMA_Matrix(n, n);
