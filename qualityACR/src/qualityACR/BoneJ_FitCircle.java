...
public class BoneJ_FitCircle {
	...
	public static double[] kasaFit(final double[][] points) {
		...
		final JAMA_Matrix XY1 = new JAMA_Matrix(xy1);
		final JAMA_Matrix Z = new JAMA_Matrix(z);
		final JAMA_Matrix P = (XY1.inverse()).times(Z); // no direct left divide in
													// Jama!
	...
	public static double[] prattNewton(final double[][] points) {
		...
		if (nPoints < 3)
			throw new IllegalArgumentException("Too few points");
		final double[] centroid = BoneJ_Centroid.getCentroid(points);
	...
	public static double[] prattSVD(final double[][] points) {
		...
		final JAMA_Matrix XYXY1 = new JAMA_Matrix(xyXY1);
		final JAMA_SingularValueDecomposition svd = new JAMA_SingularValueDecomposition(XYXY1);
		final JAMA_Matrix S = svd.getS();
		final JAMA_Matrix V = svd.getV();
		JAMA_Matrix A;
		JAMA_Matrix W;
		if (S.get(3, 3) / S.get(0, 0) < 1e-12) {
			...
			final JAMA_Matrix Binv = new JAMA_Matrix(bInv);
			final JAMA_EigenvalueDecomposition ed = new JAMA_EigenvalueDecomposition((W.transpose()).times(Binv.times(W)));
			final JAMA_Matrix D = ed.getD();
			final JAMA_Matrix E = ed.getV();
	...
	public static double[] taubinSVD(final double[][] points) {
		...
		final JAMA_Matrix ZXY = new JAMA_Matrix(zxy);
		final JAMA_SingularValueDecomposition svd = new JAMA_SingularValueDecomposition(ZXY);
		final JAMA_Matrix V = svd.getV();
		final JAMA_Matrix A = V.getMatrix(0, V.getRowDimension() - 1, 2, 2);
	...
	public static double[] hyperSimple(final double[][] points) {
		...
		final JAMA_Matrix ZXY1 = new JAMA_Matrix(zxy1);
		final JAMA_Matrix M = (ZXY1.transpose()).times(ZXY1);

		final double[][] n = { { 8 * s0, 4 * s1, 4 * s2, 2 }, { 4 * s1, 1, 0, 0 }, { 4 * s2, 0, 1, 0 },
				{ 2, 0, 0, 0 } };
		final JAMA_Matrix N = new JAMA_Matrix(n);
		final JAMA_Matrix NM = (N.inverse()).times(M);

		final JAMA_EigenvalueDecomposition ED = new JAMA_EigenvalueDecomposition(NM);
		final JAMA_Matrix E = ED.getV();
		final JAMA_Matrix D = ED.getD();

		final int col = getNthSmallestCol(D, 2);

		final JAMA_Matrix A = E.getMatrix(0, E.getRowDimension() - 1, col, col);
	...
	public static double[] hyperStable(final double[][] points) {
		...
		final JAMA_Matrix ZXY1 = new JAMA_Matrix(zxy1);
		final JAMA_SingularValueDecomposition svd = new JAMA_SingularValueDecomposition(ZXY1);
		...
		if (S.get(3, 3) / S.get(0, 0) < 1e-12) {
			...
			// regular case
			final JAMA_Matrix Y = V.times(S.times(V.transpose()));

			final double[][] bInv = { { 0, 0, 0, 0.5 }, { 0, 1, 0, 0 }, { 0, 0, 1, 0 },
					{ 0.5, 0, 0, -2 * sumZ / nPoints } };
			final JAMA_Matrix Binv = new JAMA_Matrix(bInv);
			final JAMA_EigenvalueDecomposition ED = new JAMA_EigenvalueDecomposition((Y.transpose()).times(Binv.times(Y)));
			final JAMA_Matrix D = ED.getD(); // eigenvalues
			final JAMA_Matrix E = ED.getV(); // eigenvectors
	...
	public static double[] levenMarqFull(final double[][] points, final double lambdaIni) {
		...
		JAMA_Matrix Par = new JAMA_Matrix(par);
		JAMA_Matrix ParTemp = new JAMA_Matrix(par);
		...
		final JAMA_Matrix J = new JAMA_Matrix(j);
		final JAMA_Matrix G = new JAMA_Matrix(g);
		...
		for (int iter = 0; iter < iterMax; iter++) {
			...
			while (safety < 100) {
				...
				G.set(nPoints + 1, 0, 0);
				G.set(nPoints + 2, 0, 0);
				final JAMA_Matrix DelPar = (J.inverse()).times(G);
	 ...
	 */
	private static int getNthSmallestCol(final JAMA_Matrix D, final int n) {
